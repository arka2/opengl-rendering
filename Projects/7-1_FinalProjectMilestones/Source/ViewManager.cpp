...
GLFWwindow* ViewManager::CreateDisplayWindow(const char* windowTitle)
{
	...
	// callback used to receive mouse scroll events
...
{
	// add scroll vertical offset to camera movement speed
	g_pCamera->MovementSpeed += yOffset;

	// if new speed is less than 0, set to zero
	if (g_pCamera->MovementSpeed <= 0)
	{
		g_pCamera->MovementSpeed = 0;
	}
}
...
void ViewManager::ProcessKeyboardEvents()
{
	...
	{
		// change bool to perspective projection
		bOrthographicProjection = false;

		// change camera settings to default values
		g_pCamera->Position = glm::vec3(0.0f, 5.0f, 12.0f);
		g_pCamera->Front = glm::vec3(0.0f, -0.5f, -2.0f);
		g_pCamera->Up = glm::vec3(0.0f, 1.0f, 0.0f);
	}
...
void ViewManager::PrepareSceneView(const ShaderManager &shader)
{
	...
	// edited to check for perspective or ortho perspective
